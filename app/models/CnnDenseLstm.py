...
class CnnDenseLstmOneClass(nn.Module):
    def __init__(self, input_size=3, cnn_channels=[32, 64], cnn_kernel_sizes=[3, 3],
                 lstm_hidden_size=128, lstm_layers=2, dropout=0.3):
        super().__init__()
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(64, input_size)   # 출력 feature = 입력 feature
        )
        
    def forward(self, x):
        # x: (batch, seq_len, feature) -> CNN 입력용 (batch, feature, seq_len)
        x = x.permute(0, 2, 1)
        x = self.cnn(x)               # (batch, cnn_channels[-1], seq_len)
        x = x.permute(0, 2, 1)        # (batch, seq_len, feature) -> LSTM 입력
        
        out, _ = self.lstm(x)         # (batch, seq_len, lstm_hidden_size)
